...
export default function DeliveryDashboard() {
  ...
  const { orders, updateOrder, updateOrderStatus, staff, updateStaff } = useStore();
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (username === 'domicilio' && password === 'domicilio') {
      const firstDriver = staff.find(s => s.role === 'Domiciliario');
      if (firstDriver) setLoggedInUserId(firstDriver.id);
      setIsLoggedIn(true);
      return;
    }
    
    const user = staff.find(s => (s.email === username || s.name === username) && s.password === password && s.role === 'Domiciliario');
    if (!user) {
      setLoginError('Usuario o contraseña incorrectos.');
      return;
    }
    if (!user.active) {
      setLoginError('Cuenta inactiva. Contacta al administrador.');
      return;
    }
  ...
  const executeDelivery = (orderId: string) => {
    if (loggedInUserId) {
            ...
            <button
              onClick={() => {
                ...
                if (pinInput === order?.deliveryPin) {
                  ...
                  executeDelivery(pinTargetOrder);
                } else {
