...
export default function Register() {
  ...
  const { addClient, clients, staff } = useStore();
  ...
  const {
    ...
    onSubmit: async (formValues) => {
      // Custom validations for existing email/phone
      const emailExists = clients.some(c => c.email === formValues.email) || staff.some(s => s.email === formValues.email);
      const phoneExists = clients.some(c => c.phone === formValues.telefono) || staff.some(s => (s as any).phone === formValues.telefono);
      
      let hasError = false;
      if (emailExists) {
        setFieldError('email', 'Este correo ya está registrado. Inicia sesión o recupera tu contraseña.');
        hasError = true;
      }
      if (phoneExists) {
        setFieldError('telefono', 'Este teléfono ya está registrado. Inicia sesión o recupera tu contraseña.');
        hasError = true;
      }
      if (hasError) return;

      // Simular tiempo de carga del registro
      ...
      localStorage.setItem('copiway_auth_user_id', newClientId);
      localStorage.setItem('copiway_auth_role', 'client');
      setSubmitSuccess(true);
      setTimeout(() => {
        setSubmitSuccess(false);
        navigate('/client');
      }, 1500); // Reduce the delay to 1.5s for a smoother UX and then redirect
    },
  });
