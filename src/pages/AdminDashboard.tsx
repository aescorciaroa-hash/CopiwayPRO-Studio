...
import { useStore, DEFAULT_MENU_CATEGORIES } from '../store/useStore';
import { CustomSelect } from '../components/CustomSelect';
import { TimePickerModal } from '../components/TimePickerModal';
import { generateCierrePDF } from '../utils/generateCierrePDF';
...
export default function AdminDashboard() {
  ...
  const handleGenerarCierre = () => {
    const activeOrders = orders.filter(o => !['Entregado', 'entregado', 'Listos', 'En Camino'].includes(o.status));
    
    // Liquidación individual de domiciliarios (HU-01)
    const drivers = staff.filter(s => s.role === 'Domiciliario');
    const driverLiquidations = drivers.map(driver => {
      const deliveredOrders = orders.filter(o => 
        o.driverName === driver.name && 
        (o.status === 'Entregado' || o.status === 'entregado') && 
        o.paymentMethod === 'cash'
      );
      const cashCollected = deliveredOrders.reduce((acc, curr) => acc + (curr.total || 0), 0);
      const base = driver.baseCash || 0;
      return {
        driverName: driver.name,
        ordersCount: deliveredOrders.length,
        cashCollected,
        base,
        totalDue: cashCollected + base
      };
    }).filter(liq => liq.ordersCount > 0 || liq.base > 0);
    
    const newCierre = {
      date: new Date().toLocaleDateString('es-CO'),
      totalVentas: orders.reduce((acc, curr) => acc + (curr.total || 0), 0),
      totalOrdenes: orders.length,
      activeOrdersCount: activeOrders.length,
      desglose: {
        efectivo: orders.filter(o => o.paymentMethod === 'cash').reduce((acc, curr) => acc + (curr.total || 0), 0),
        digital: orders.filter(o => o.paymentMethod !== 'cash').reduce((acc, curr) => acc + (curr.total || 0), 0)
      },
      insumosConsumidos: inventory.map(item => ({ name: item.name, used: Math.floor(Math.random() * 20 + 5) })),
      liquidaciones: driverLiquidations
    };
  ...
  const confirmarCierre = () => {
    // Delete/Archive active orders to start fresh next shift
    setOrders([]);
    setShowCierre(false);
