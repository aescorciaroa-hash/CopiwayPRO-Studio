...
export default function Login() {
  ...
  const {
    ...
    onSubmit: async (formValues) => {
      ...
      const { staff, clients } = useStore.getState();
      const staffMember = staff.find(s => s.email === formValues.email && s.password === formValues.password && s.active);

      if ((formValues.email === 'aescorciaroa@gmil.com' || formValues.email === 'aescorciaroa@gmail.com') && formValues.password === '25072007') {
        localStorage.setItem('copiway_auth_user_id', 'admin');
        localStorage.setItem('copiway_auth_role', 'admin');
        setSubmitSuccess(true);
        setTimeout(() => {
          navigate('/admin');
        }, 1500);
      } else if (formValues.email === 'majo@gmail.com') {
        localStorage.setItem('copiway_auth_user_id', 'kitchen');
        localStorage.setItem('copiway_auth_role', 'kitchen');
        setSubmitSuccess(true);
        setTimeout(() => {
          navigate('/kitchen');
        }, 1500);
      } else if (formValues.email === 'camilo@gmail.com') {
        localStorage.setItem('copiway_auth_user_id', 'delivery');
        localStorage.setItem('copiway_auth_role', 'delivery');
        setSubmitSuccess(true);
        setTimeout(() => {
          navigate('/delivery');
        }, 1500);
      } else if (staffMember) {
        localStorage.setItem('copiway_auth_user_id', staffMember.id);
        localStorage.setItem('copiway_auth_role', staffMember.role);
        setSubmitSuccess(true);
        setTimeout(() => {
          if (staffMember.role === 'Ayudante de cocina') {
            navigate('/kitchen');
          } else if (staffMember.role === 'Domiciliario') {
            navigate('/delivery');
          } else {
            navigate('/client');
          }
        }, 1500);
      } else if (formValues.email === 'cocina@copiway.com' && formValues.password === '12345678') {
        localStorage.setItem('copiway_auth_user_id', 'kitchen');
        localStorage.setItem('copiway_auth_role', 'kitchen');
        setSubmitSuccess(true);
        setTimeout(() => {
      ...
      } else if (formValues.email === 'domiciliario@copiway.com' && formValues.password === '12345678') {
        localStorage.setItem('copiway_auth_user_id', 'delivery');
        localStorage.setItem('copiway_auth_role', 'delivery');
        setSubmitSuccess(true);
        setTimeout(() => {
      ...
      } else if (formValues.email === 'aescorciaroa@gmil.com' && formValues.password !== '25072007') {
         setLoginError('Contraseña de administrador incorrecta. (Pista: 25072007)');
      } else if (formValues.email === 'aescorciaroa@gmail.com' && formValues.password !== '25072007') {
         setLoginError('Contraseña de administrador incorrecta. (Pista: 25072007)');
      } else {
        const client = clients.find(c => c.email === formValues.email);
        if (client) {
          localStorage.setItem('copiway_auth_user_id', client.id);
          localStorage.setItem('copiway_auth_role', 'client');
        } else {
          localStorage.setItem('copiway_auth_user_id', 'guest');
          localStorage.setItem('copiway_auth_role', 'client');
        }
        setSubmitSuccess(true);
        setTimeout(() => {
